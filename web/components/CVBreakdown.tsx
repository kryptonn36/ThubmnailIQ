...
export default function CVBreakdown({ cvResults, competitorAvg }: CVBreakdownProps) {
  return (
    <div className="rounded-xl border border-surface-300 bg-surface-100 p-5">
      ...
      <div>
        <StatRow label="Face count" value={String(cvResults.face.face_count)} compare={competitorAvg ? `avg ${competitorAvg.face_count}` : undefined} />
        <StatRow label="Primary emotion" value={cvResults.face.primary_emotion ?? "None detected"} />
        <StatRow
          label="Text detected"
          value={cvResults.ocr.text_detected ? "Yes" : "No"}
        />
        <StatRow
          label="Text density"
